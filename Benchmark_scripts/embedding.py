...
def create_embedding_model(vocabulary, demographic_size, embedding_dim=1024):
    ...
    model = Model(inputs=[demograph_input, icd_input], outputs=predict, name="embedding_model")
    return model
...
def create_base_model(vocabulary, demographic_size, embedding_dim=1024):
    ...
    model = Model(inputs=[demograph_input, icd_input], outputs=predict, name="base_model")
    return model
...
def setup_embedding_data(df_train_embed, df_test_embed, X_train, y_train, X_test, y_test, batch_size):
    ...
    # Safely parse icd_encoded_list
    icd_train_list = [safe_parse_icd(x) for x in df_train_embed['icd_encoded_list'].copy()]
    icd_test_list = [safe_parse_icd(x) for x in df_test_embed['icd_encoded_list'].copy()]

    # Ensure all data is numpy float32
    X_train = X_train.to_numpy(dtype=np.float32)
    y_train = y_train.to_numpy(dtype=np.float32)
    X_test = X_test.to_numpy(dtype=np.float32)
    y_test = y_test.to_numpy(dtype=np.float32)
